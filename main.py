...
MODE_COLOR = "Color Detection"
MODE_TEXT  = "Text Detection"

MATCH_CONTAINS = "Contains"
MATCH_EXACT    = "Exact"
MATCH_REGEX    = "Regex"

COLOR_DEFAULT_MSG = "A screen change has been detected."
...
class ColorMonitorWorker(QObject):
    ...
    def __init__(self, area: QRect, bot_token: str, chat_ids: list,
        ...
        self.message   = message or COLOR_DEFAULT_MSG
        ...
        try:
            with mss.mss() as sct:
                ...
                while self._running:
                    ...
                    diff    = np.abs(current.astype(np.int32) - prev.astype(np.int32))
                    changed = int(np.sum(diff.max(axis=2) > 10))

                    if changed > 0:
                        self.log_signal.emit(
                            f"Change detected! ({changed} pixels)  "
...
class TextMonitorWorker(QObject):
    ...
    def __init__(self, area: QRect, bot_token: str, chat_ids: list,
        ...
        self.interval   = interval
        self._running   = False
        self._last_sent: Optional[str] = last_sent or None
...
class SettingsDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        self.message_input.setPlaceholderText(
            'Empty → color: default alert / text: detected text'
        )
        ...
        # ── Text Detection (shown only in text mode) ───────────────────────────
        self.det_box    = QGroupBox("Text Detection")
        det_layout = QVBoxLayout(self.det_box)
        ...
        self.det_box.setVisible(self._main._mode == MODE_TEXT)
        root.addWidget(self.det_box)
    ...
    def _refresh_area_label(self):
        if self._area:
            r = self._area
            self.area_label.setText(f"({r.x()}, {r.y()})  {r.width()} × {r.height()} px")
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Screen Detect Bot")
    ...
    def _build_ui(self):
        ...
        self.mode_combo = QComboBox()
        self.mode_combo.addItems([MODE_COLOR, MODE_TEXT])
    ...
    def _on_start(self):
        ...
        if not self.selected_area:
            ...
            return
        if self._mode == MODE_TEXT and not cfg["targets"]:
            self._log("Open Settings and add at least one target text.")
            return

        if self._mode == MODE_COLOR:
            self.worker = ColorMonitorWorker(
                self.selected_area,
                cfg["token"], cfg["chat_ids"],
                cfg["message"], cfg["interval"],
            )
        else:
        ...
        if isinstance(self.worker, TextMonitorWorker):
    ...
    def _refresh_start_btn(self):
        ...
        if self._mode == MODE_TEXT:
            ready = ready and bool(cfg.get("targets"))
    ...
    def _load_settings(self):
        ...
        if s.get("AREA"):
            try:
                x, y, w, h = (int(v) for v in s["AREA"].split(","))
                self.selected_area = QRect(x, y, w, h)
            except (ValueError, TypeError):
                pass
        if s.get("MODE"):
            self._mode = s["MODE"]
            idx = self.mode_combo.findText(self._mode)
            if idx >= 0:
                self.mode_combo.setCurrentIndex(idx)
        self._refresh_start_btn()
    ...
    def _save_settings(self):
        ...
        storage.save_settings({
            ...
            "MODE":          self._mode,
            "AREA":          (
                f"{self.selected_area.x()},{self.selected_area.y()},"
                f"{self.selected_area.width()},{self.selected_area.height()}"
                if self.selected_area else ""
            ),
        })
